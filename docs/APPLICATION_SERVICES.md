...
task. It implements read-only mapping access so legacy runner code can continue
using `task["id"]` while new code can use typed attributes such as `task.id`.
It does not expose `sqlite3.Row` outside the repository.
